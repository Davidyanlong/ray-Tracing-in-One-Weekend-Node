...
function ray_color(r:Ray, world:Hitable, depth:number) {
    ...
    if (world.hit(r, 0.001, infinity, rec)) {
        ...
        if (rec.mat_ptr?.scatter(r, rec, attenuation, scattered))
            return attenuation.clone().multiply(
...
function ray_color2(r, world, depth) {
  ...
  if (world.hit(r, 0.001, infinity, rec)) {
    ...
    let attenuation = new Color();
    if (rec.mat_ptr.scatter(r, rec, attenuation, scattered)) {
      return attenuation.multiplyVector3(
        ray_color(scattered, world, depth - 1)
      );
    }
    return new Color(0, 0, 0);
  ...
  let color2 = new Color(0.5, 0.7, 1.0);

  color1.multiply(1 - t);
  color2.multiply(t);

  return Vector3.add(color1, color2);
}

function random_scene() {
  let world = new HittableList();
  let material_ground = MakeShared(new Lambertian(new Color(0.5, 0.5, 0.5)));
  world.add(
    MakeShared(
  ...
  for (let a = -11; a < 11; a++) {
    for (let b = -11; b < 11; b++) {
      ...
      if (Vector3.sub(center, new Vector3(4, 0.2, 0)).length() > 0.9) {
        let sphere_material = new Material();
        if (choose_mat < 0.8) {
          // diffuse
          let albedo = Vector3.multiplyVector3(Color.random(), Color.random());
          sphere_material = MakeShared(new Lambertian(albedo));
        ...
        world.add(MakeShared(new Sphere(center, 0.2, sphere_material)));
      }
    }
  }
  ...
  let material2 = MakeShared(new Lambertian(new Color(0.4, 0.2, 0.1)));
  world.add(MakeShared(new Sphere(new Point3(-4, 1, 0), 1.0, material2)));
...
function main() {
  ...
  // Camera
  const lookfrom = new Point3(12, 2, 3);
